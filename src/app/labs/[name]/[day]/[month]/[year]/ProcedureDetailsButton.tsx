...
export function ProcedureDetailsButton(
    props: { 
        ...
        <>
            ...
            >
                ...
                >
                        ...
                        <p className="bg-gray-300">{props.procedure.submiter.name}</p>
                        ...
                        <p className="bg-gray-300">{props.procedure.start_date.toLocaleDateString('es', {
                            hour: '2-digit',
                            minute: '2-digit',
                        })}</p>
                        <span className="text-right">Duración:</span>
                        <p>{props.procedure.end_date.getHours() - props.procedure.start_date.getHours()} horas</p>
                        {props.procedure.UsedTool.length ? (
                            <div className="col-span-2">
                                {props.procedure.UsedTool.map(t => (
                                    <article key={t.tool.id}>
                                        <p className="">{t.tool.name} x{t.quantity}</p>
                                    </article>
                                ))}
                            </div>
                        ) : null}
