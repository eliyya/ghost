...
export function EditLabForm(props: EditLabFormProps) {
    ...
    const dateToTime = (date: Date) => `${date.getUTCHours().toString().padStart(2, '0')}:${date.getUTCMinutes().toString().padStart(2, '0')}`

    return (
        <form
            action={async e => {
                const name = e.get('name') as string
                const open_date =  new Date('2024-01-01T'+e.get('open_date')+':00.000Z')
                const close_date =  new Date('2024-01-01T'+e.get('close_date')+':00.000Z')
